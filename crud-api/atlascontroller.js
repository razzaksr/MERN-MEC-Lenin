...
// Routes/ endpoints
atlasrouter.get('/', tokenVerification,roleVerification(['customer','officials']) ,async(req, res) => {
    const records = await dao.getAllRecords();
    res.json(records);
});

atlasrouter.post('/', tokenVerification, roleVerification(['officials']) ,async(req, res) => {
    const newRecord = req.body;
    const createdRecord = await dao.createRecord(newRecord);
    res.json(createdRecord);
});

atlasrouter.patch('/:usereg', tokenVerification, roleVerification(['officials']) ,async(req, res) => {
...
atlasrouter.delete('/:usereg', tokenVerification, roleVerification(['officials']) ,async(req, res) => {
    const usereg = req.params.usereg;
